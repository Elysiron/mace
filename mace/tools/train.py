...
def valid_err_log(valid_loss, eval_metrics, logger, log_errors, epoch=None):
    ...
    if log_errors == "PerAtomRMSE":
        ...
        log_errors == "PerAtomRMSEstressvirials"
        and eval_metrics["rmse_virials"] is not None
    ):
        error_e = eval_metrics["rmse_e_per_atom"] * 1e3
        error_f = eval_metrics["rmse_f"] * 1e3
        error_virials = eval_metrics["rmse_virials"] * 1e3
        logging.info(
            f"Epoch {epoch}: loss={valid_loss:.4f}, RMSE_E_per_atom={error_e:.1f} meV, RMSE_F={error_f:.1f} meV / A, RMSE_virials={error_virials:.1f} meV"
        )
